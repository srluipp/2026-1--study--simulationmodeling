Ниже представлен подробный отчёт по теме **«Образование планетной системы»**, оформленный в формате Markdown. Он включает теоретическую базу, физические основы моделирования, математические модели и описание этапов численного эксперимента.

---

# Отчёт по моделированию образования планетной системы

## 1. Введение

Формирование планетных систем — один из ключевых процессов эволюции Вселенной. Согласно современным представлениям, звёзды и их планетные системы образуются в результате гравитационного сжатия межзвёздных газопылевых облаков. В процессе сжатия облако фрагментируется, формируются протозвёзды и протопланетные диски, в которых впоследствии образуются планеты.

Цель данной работы — численное моделирование образования планетной системы из газопылевого облака с использованием методов молекулярной динамики, гравитационного взаимодействия, сил трения и слипания частиц.

---

## 2. Теоретическая база

### 2.1. Космологический контекст

Согласно теории Большого взрыва, Вселенная возникла около 13,7 млрд лет назад из сингулярного состояния. В процессе расширения и охлаждения образовались элементарные частицы, затем атомы, и под действием гравитационной неустойчивости начали формироваться первые структуры: протоскопления, протогалактики, галактики и, наконец, звёзды.

Звёзды, превышающие массу Солнца в десятки раз, быстро эволюционируют и взрываются как сверхновые, выбрасывая тяжёлые элементы, из которых впоследствии формируются новые звёзды и планеты.

### 2.2. Формирование Солнечной системы

Солнечная система образовалась из вращающегося газопылевого облака. Под действием гравитации облако сжималось, его момент импульса сохранялся, что привело к увеличению скорости вращения и сплющиванию облака в диск. В диске возникали кольцевые структуры, которые затем сгущались в планеты. Скорость движения планет подчиняется третьему закону Кеплера:

$$
v \sim \frac{1}{\sqrt{r}}
$$

где$r$— расстояние до центра.

---

## 3. Математическая модель

### 3.1. Гравитационное взаимодействие

Потенциальная энергия гравитационного взаимодействия системы частиц:

$$
U_i = -\sum_{j \neq i} \frac{\gamma m_i m_j}{r_{ij}}, \quad U = \frac{1}{2} \sum_i U_i
$$

где$\gamma$— гравитационная постоянная,$m_i$,$m_j$— массы частиц,$r_{ij}$— расстояние между ними.

Гравитация — дальнодействующая сила, что требует учёта взаимодействий между всеми парами частиц. Это приводит к сложности$O(N^2)$, что ограничивает размер моделируемых систем.

### 3.2. Начальные условия

Частицы распределяются случайным образом в плоскости диска:

- радиус:$r = r_0 \cdot \text{random}$
- угол:$\alpha = 2\pi \cdot \text{random}$
- начальная скорость:

$$
v_x = -y \cdot \omega_0 \left( \frac{r_0}{r} \right)^{3/2}, \quad v_y = x \cdot \omega_0 \left( \frac{r_0}{r} \right)^{3/2}, \quad v_z = 0
$$

где$\omega_0$— угловая скорость на расстоянии$r_0$.

### 3.3. Силы отталкивания и трения

При сближении частиц на расстояние, меньшее суммы их радиусов, вводятся силы отталкивания и трения.

#### Сила отталкивания:

$$
F^r(b) = k \left( \left( \frac{a}{b} \right)^8 - 1 \right)
$$

где$b$— расстояние между центрами частиц,$a$— характерный масштаб.

#### Сила трения (2D):

Единичный вектор направления трения:

$$
n = \frac{(-b_y, b_x)}{\sqrt{b_x^2 + b_y^2}}
$$

Относительная скорость поверхностей:

$$
W_\perp = (v_i - v_j) \cdot n - \omega_i R_i - \omega_j R_j
$$

Модуль силы трения:

$$
F^f = \mu F^r(b), \quad \mu = \beta W_\perp
$$

Вектор силы трения:

$$
\mathbf{F}_f = \beta W_\perp F^r(b) \mathbf{n}
$$

### 3.4. Вращение частиц

Уравнения вращения:

$$
I_i \varepsilon_i = R_i \sum \frac{b}{R_i + R_j} F^f_{ij}, \quad \frac{d\omega_i}{dt} = \varepsilon_i
$$

Момент инерции:

$$
I_i = \frac{2}{5} m_i R_i^2
$$

Энергия вращения:

$$
E_{\text{rot}} = \frac{I_i \omega_i^2}{2}
$$

### 3.5. Слипание частиц

При полном слипании:

- радиус:$R = \sqrt[3]{R_i^3 + R_j^3}$
- масса:$m = m_i + m_j$
- координаты:$\mathbf{r} = \frac{m_i \mathbf{r}_i + m_j \mathbf{r}_j}{m_i + m_j}$
- скорость:$\mathbf{v} = \frac{m_i \mathbf{v}_i + m_j \mathbf{v}_j}{m_i + m_j}$

Гравитационная энергия новой частицы:

$$
E_g = -\frac{\gamma m^2}{2R}
$$

не равна сумме энергий исходных частиц.

---

## 4. Этапы моделирования

### 4.1. Двумерная модель с центральным притяжением

На первом этапе моделируется движение частиц в плоскости под действием центральной неподвижной массы. Частицы движутся по орбитам с первой космической скоростью, не взаимодействуя друг с другом.

### 4.2. Введение гравитации и отталкивания

Убирается центральная точка, вводится гравитационное взаимодействие между частицами. При сближении частиц на расстояние, меньшее суммы радиусов, включается сила отталкивания. Контролируется полный импульс системы (должен быть равен нулю). На экран выводятся кинетическая и потенциальная энергии.

### 4.3. Учёт вращения

Добавляются угловые скорости вращения частиц вокруг собственной оси. Вводится сила трения.

### 4.4. Трёхмерная модель

Моделирование переносится в трёхмерное пространство. Выводятся проекции движения на плоскости$XY$,$XZ$,$YZ$. Строятся графики зависимости энергий от времени.

### 4.5. Слипание частиц

В трёхмерной модели вводится механизм слипания частиц при их сближении. Сохраняются масса и импульс системы.

### 4.6. Трение и диссипация энергии

Вводится сила трения. Анализируется поведение кинетической и потенциальной энергии. Часть энергии переходит в тепло.

### 4.7. Частицы разных сортов

Вводятся частицы двух типов с разной массой и радиусом. Добавляется трение. Выводится график энергии, переходящей в тепло. Затем массы и радиусы задаются случайным образом.

---

## 5. Заключение

В результате выполнения работы была построена численная модель формирования планетной системы из газопылевого облака. Модель включает гравитационное взаимодействие, силы отталкивания и трения, вращение и слипание частиц. Такой подход позволяет исследовать динамику системы на разных этапах её эволюции и наблюдать образование сгущений — прообразов планет.

Модель может быть расширена за счёт использования более эффективных алгоритмов (например, мультипольных разложений) для увеличения числа частиц и повышения реалистичности.

---

Если хотите, я могу также:
- написать программную реализацию на Python или C++,
- подготовить презентацию,
- оформить отчёт в PDF,
- добавить графики и визуализацию.